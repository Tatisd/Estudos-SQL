...
left join temp_tables.ibge_genders as ibge  --nessa tabela está listado os nome e gêneros atribuidos a eles
	on lower(cus.first_name) = lower(ibge.first_name) --puxa o pimeiros nome da tabela que chamamos de cus e da tabela que chamamos de ibge colocando tudo em letras minusculas
...
select
	case
		...
		when professional_status = 'other' then 'ourto'
		when professional_status = 'businessman' then 'empresário(a)'
		when professional_status = 'civil_servant' then 'funcionário publico'
		when professional_status = 'student' then 'estudante'
		end as "status profissional",
	(count(*)::float)/(select count(*) from sales.customers) as "leads (%)" -- a segunda parte da querie retorna a qtidade de leads da tabela, nesse caso 25109
...
-- Regra de negócio: Veículos novos tem até 2 anos e seminovos acima de 2 anos (dado fornecido pela empresa)
